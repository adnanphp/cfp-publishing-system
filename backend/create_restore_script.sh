...
BACKUP_DIRS=$(find . -name "src.backup*" -type d 2>/dev/null)

if [ -z "$BACKUP_DIRS" ]; then
    echo "No backup directories found. Looking for any backup..."
    BACKUP_DIRS=$(find . -name "*backup*" -type d 2>/dev/null | head -1)
fi

if [ -z "$BACKUP_DIRS" ]; then
    echo "No backup found. Please specify backup directory manually."
    echo "Example: ./restore_files.sh /path/to/backup"
    exit 1
fi

# Use the most recent backup
BACKUP=$(echo "$BACKUP_DIRS" | sort -r | head -1)
echo "Using backup: $BACKUP"

echo -e "\nListing available files in backup:"
find "$BACKUP" -name "*.rs" | head -30

echo -e "\nCreating restoration script..."
cat > restore_gradually.sh << 'EOF'
#!/bin/bash
...
chmod +x restore_gradually.sh

echo -e "\nCreated restoration script: restore_gradually.sh"
echo "Run it with: ./restore_gradually.sh '$BACKUP'"
